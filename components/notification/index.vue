<template>
    <div class="toast card text-neutral-content m-0 me-1 p-0">
        <div v-for="(item, index) in NotificationManager.notificationShowList.value" :key="index"
            class="flex relative flex-row card-body text-left w-96 p-3 mb-2 bg-neutral rounded-btn">
            <div class="flex-1 m-3">
                ...
                <div class="card-actions justify-end">
                    <button v-if="item.okHandler" @click="close(item, item.okHandler.handler)"
                        class="btn btn-primary btn-sm">{{
                            item.okHandler.text }}</button>
                    <button v-if="item.cancelHandler" @click="close(item, item.cancelHandler.handler)"
                        class="btn btn-ghost btn-sm">{{ item.cancelHandler.text }}</button>
                </div>
            </div>
            <div class="absolute top-3 right-3">
                <svg @click="close(item)" class="hover:bg-base-100 rounded-full" xmlns="http://www.w3.org/2000/svg"
...
function close(notification, handler: Function = () => { }) {
    NotificationManager.closeNotificationById(notification.id);
    handler();
}
</script>
